...
def IsConnected2(Source, Sourcepos, Target, Targetpos, d,Col_L, Layer_d):
...
def Create_Connectivity_Matrix(C,inputpercent,NB_DPYR,NB_Th,Cellpos,seed=0,func=None):
    ...
    for l in range(len(C.Layer_nbCells)):
        for cell in range(int(C.Layer_nbCells[l])):
            ...
            elif np.sum(cm[0:C.NB_RLN[0]]) < Afferences[4]:# and not np.sum(cm[0:C.NB_RLN[0]]) == 0:
                ...
                for type in range(5):
                    if not int(Afferences[type + 5 * ll]) == 0:
                        ...
                        if len(NBrange) > 0:
                            ...
                            elif somme_cm < int(Afferences[type + 5 * ll]):# and not somme_cm == 0:
...
def Computeinputdistribution(PreSynapticPos_AMPA,PresynapticPos_GABA,C):
    #thiss function computes the distribution of excitatory and inhibitory input per cell type and layer
    PC_AMPA_PER=[]
    PC_GABA_PER=[]
    sumLayer_nbCells = np.cumsum(C.Layer_nbCells)
    for l in range(1,len(C.Layer_nbCells)):
        PC_AMPA = np.zeros(5)
        PC_GABA = np.zeros(5)

        # for i in range(sumLayer_nbCells[l-1],sumLayer_nbCells[l-1]+C.NB_PYR[l]):
        #
        #     if not PreSynapticPos_AMPA == []:
        #         PC_AMPA[0]+= np.sum(PreSynapticPos_AMPA[i] == 5)/len(PreSynapticPos_AMPA[i])
        #         PC_AMPA[1]+= np.sum(PreSynapticPos_AMPA[i] == 4)/len(PreSynapticPos_AMPA[i])
        #         PC_AMPA[2]+= np.sum(PreSynapticPos_AMPA[i] == 3)/len(PreSynapticPos_AMPA[i])
        #         PC_AMPA[3]+= np.sum(PreSynapticPos_AMPA[i] == 2)/len(PreSynapticPos_AMPA[i])
        #         PC_AMPA[4]+= np.sum(PreSynapticPos_AMPA[i] == 1)/len(PreSynapticPos_AMPA[i])
        #
        #     if not PresynapticPos_GABA == []:
        #         PC_GABA[0]+=np.sum(PresynapticPos_GABA[i] == 5)/len(PresynapticPos_GABA[i])
        #         PC_GABA[1]+= np.sum(PresynapticPos_GABA[i] == 4)/len(PresynapticPos_GABA[i])
        #         PC_GABA[2]+= np.sum(PresynapticPos_GABA[i] == 3)/len(PresynapticPos_GABA[i])
        #         PC_GABA[3]+= np.sum(PresynapticPos_GABA[i] == 2)/len(PresynapticPos_GABA[i])
        #         PC_GABA[4]+= np.sum(PresynapticPos_GABA[i] == 1)/len(PresynapticPos_GABA[i])

        arr = np.concatenate(PreSynapticPos_AMPA[sumLayer_nbCells[l - 1]:sumLayer_nbCells[l - 1] + C.NB_PYR[l]]).ravel()
        sumarr = len(arr)
        PC_AMPA[0] = np.sum(arr == 5)/sumarr
        PC_AMPA[1] = np.sum(arr == 4)/sumarr
        PC_AMPA[2] = np.sum(arr == 3)/sumarr
        PC_AMPA[3] = np.sum(arr == 2)/sumarr
        PC_AMPA[4] = np.sum(arr == 1)/sumarr
        print(np.sum(PC_AMPA))

        arr = np.concatenate(PresynapticPos_GABA[sumLayer_nbCells[l - 1]:sumLayer_nbCells[l - 1] + C.NB_PYR[l]]).ravel()
        sumarr = len(arr)
        PC_GABA[0] = np.sum(arr == 5)/sumarr
        PC_GABA[1] = np.sum(arr == 4)/sumarr
        PC_GABA[2] = np.sum(arr == 3)/sumarr
        PC_GABA[3] = np.sum(arr == 2)/sumarr
        PC_GABA[4] = np.sum(arr == 1)/sumarr
        print(np.sum(PC_GABA))

        # PC_AMPA=PC_AMPA/C.NB_PYR[l]
        # PC_GABA=PC_GABA/C.NB_PYR[l]


        PC_AMPA_PER.append(PC_AMPA)
        PC_GABA_PER.append(PC_GABA)

    return PC_AMPA_PER, PC_GABA_PER
