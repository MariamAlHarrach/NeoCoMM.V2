...
def Model_compute(nbEch,
    ...
    for tt, tp in enumerate(t):
        ...
        for i in range(np.sum(NB_PYR)):# for EField
            ...
            # print(List_PYR[i].Lambda_s[2])
            # print([List_PYR[i].Vtm_s,List_PYR[i].Vtm_d,List_PYR[i].Vtm_a])

        for i in range(NB_DPYR):# for EField
            List_DPYR[i].update_EField_Stim(Stim_EField[tt])

        for i in range(NB_Th):# for EField
            List_Th[i].update_EField_Stim(Stim_EField[tt])

        for i in range(np.sum(NB_PV)):# for EField
            List_PV[i].update_EField_Stim(Stim_EField[tt])

        for i in range(np.sum(NB_SST)):# for EField
            List_SST[i].update_EField_Stim(Stim_EField[tt])

        for i in range(np.sum(NB_VIP)):# for EField
            List_VIP[i].update_EField_Stim(Stim_EField[tt])

        for i in range(np.sum(NB_RLN)):# for EField
            List_RLN[i].update_EField_Stim(Stim_EField[tt])
        ...
        for i in range(np.sum(Layer_nbCells)):
            ...
            if len(PS.PreSynaptic_Cell_AMPA[i]) > 0 or len(PS.ExternalPreSynaptic_Cell_AMPA_DPYR[i]) > 0 or len(PS.ExternalPreSynaptic_Cell_AMPA_Th[i]) > 0:
                ...
                if typeS[i] == 0:  # PC
                    ...
                    pyrI_S[curr_pyr, tt]=- List_PYR[neurone].I_synSoma - (List_PYR[neurone].g_c / List_PYR[neurone].p_SD * ((List_PYR[neurone].Vs_+List_PYR[neurone].Vtm_s)- (List_PYR[neurone].Vd_+List_PYR[neurone].Vtm_d))) - (List_PYR[neurone].g_c / List_PYR[neurone].p_SA * ((List_PYR[neurone].Vs_+List_PYR[neurone].Vtm_s)- (List_PYR[neurone].Va_+List_PYR[neurone].Vtm_a)))
                    pyrI_d[curr_pyr, tt]=- List_PYR[neurone].I_synDend - (List_PYR[neurone].g_c / (1 - List_PYR[neurone].p_SD) * (-(List_PYR[neurone].Vs_+List_PYR[neurone].Vtm_s) + (List_PYR[neurone].Vd_+List_PYR[neurone].Vtm_d)))
                    pyrI_A[curr_pyr, tt]=- List_PYR[neurone].I_synAis  - (List_PYR[neurone].g_c / (1 - List_PYR[neurone].p_SA) * ( (List_PYR[neurone].Va_ + List_PYR[neurone].Vtm_a) - (List_PYR[neurone].Vs_ + List_PYR[neurone].Vtm_s)))
        ...
        for i in range(np.sum(NB_PYR)):
            ...
            W_plasticity[i, tt] = List_PYR[i].y[29]
        for i in range(np.sum(NB_PV)):
    ...
    return t,pyrVs, W_plasticity, pyrVd, pyrVa, PV_Vs, SST_Vs, VIP_Vs, RLN_Vs, DPYR_Vs, Th_Vs, pyrPPSE, pyrPPSI, pyrPPSI_s, pyrPPSI_a, pyrPPSE_Dpyr, pyrPPSE_Th, pyrI_S, pyrI_d, pyrI_A
