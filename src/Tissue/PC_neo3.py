...
def get_Variable_Names():
    return  ['g_leak_s','g_leak_d','g_leak_a','g_Na_s','g_KDR_s','g_CaL_s','g_AHP_s','g_AHP_BK_s','g_m_s','g_KNa',
             ...
             'g_c','PERCENT', 'theta_d', 'theta_p','taux_W', 'beta_1', 'beta_2', 'A', 'Bb','omega0','Lamda', 'W_max','plasticity','initial_Brunel','initial_Shouval','initial_noplasticity']
...
# I_h: hyperpolarisation activated current
    ...
    def __init__(self,Layer,plasticity=0):
        ...
        self.theta_d = 0.3  #0.3                # 1 Brunel # 0.3 shouval #0.2 pour dep, the value is in M, later we multply bith theta_d*1e-3 to become mM as everywhere else it's in mM in the code
        self.theta_p = 0.55 #0.55                # 1.3 Brunel  # 0.55 shouval #0.35 pour pot
        self.beta_1 = 80 #80
        self.beta_2 = 80 #80
        self.A = 1   #550 # 5  # 1               # 550 Brunel
        self.Bb = 0.3   #250  # 1 # 0.25         # 250 Brunel
        self.omega0 = 0.25 #0.25  # 1 # 0.25     =0 in case of Brunel
        self.Lamda = 1
        self.W_max = 1
        self.TauxCa = 20
        self.taux_W = 2000              #2000 #time constant of synaptic efficacy (seconds to minutes) [Brunel, Graupner 2012]
        self.W_str= 0.35 # the boundary of the two stable states (depression/potentiation) [Brunel, Graupner 2012]
        self.plasticity = plasticity # if =1 use brunel w, if =0 use shouval
        self.initial_Brunel = 0.5
        self.initial_Shouval = 0.25
    ...
    def setParameters(self):
        ...
        if self.plasticity==0:
            ...
            self.y[29] = self.initial_Shouval
        elif self.plasticity==2:
    ...
    def derivT(self):
        ...
        self.dVs_dt = 1. / self.Cm * (-self.I_KNa - self.I_Na_s - self.I_KDR_s - self.I_CaL_s - self.I_AHP_s - self.I_AHP_BK_s - self.I_m_s - self.I_leak_s - self.I_synSoma - (self.g_c / self.p_SD * ((self.Vs_+self.Vtm_s)- (self.Vd_+self.Vtm_d))) - (self.g_c / self.p_SA * ((self.Vs_+self.Vtm_s)- (self.Va_+self.Vtm_a))) + self.I_soma_stim)
        ...
        self.dCa_d_dt = self.drive_channel_d + ((0.0001 - self.Ca_d_) / self.TauxCa) * 1. + (self.I_NMDA_CA1 * 1e-6) / (2 * self.F)      #New Calcium_Shouval
        ###################################################################################################################
        ## Synaptic Weight variation
        if self.plasticity==0:
            self.dW_dt = 0   #1
        elif self.plasticity==1:
            self.dW_dt = self.eta_func(self.Ca_d_*1e3) * (self.W_max *self.omega_func(self.Ca_d_*1e3) - self.Lamda*self.W_) # Shouval
        elif self.plasticity==2:
            #self.dW_dt = 1/self.taux_W*(-self.W_*(1-self.W_)*(self.W_str-self.W_) + ((1-self.W_)*self.omega_p - (self.W_*self.omega_d)) + self.omega0 )             # Brunel, Shouval             #*self.eta_func(self.Ca_d_*1e3)
            #self.dW_dt = 1/self.taux_W*(-self.W_*(1-self.W_)*(self.W_str-self.W_) + ((1-self.W_)*self.A * self.Hevi_p(self.Ca_d_*1e3) - (self.W_*self.Bb * self.Hevi_d(self.Ca_d_*1e3)) + self.omega0 ))            # Brunel with Heaviside
            self.dW_dt = 1/self.taux_W*(-self.W_*(1-self.W_)*(self.W_str-self.W_) + ((1-self.W_)* self.omega_p(self.Ca_d_*1e3) - (self.W_* self.omega_d(self.Ca_d_*1e3))))              # Brunel/Shouval(New Brunel) I removed the omega0
        ###################################################################################################################  
    ...
    def computeI_NMDA(self,Vpre,t,Vs_d):
        ...
        return self.Ra * self.B * self.g_NMDA * (Vs_d - self.E_NMDA)* self.y[29]
