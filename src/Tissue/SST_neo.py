...
class SSTcell:
    ...
    def updateParameters(self):
        self.m_inf_Na = self.m_inf_Na_OLM((self.y[0] + self.Vtm_s))
        self.m_inf_Ca = self.m_inf_Ca_OLM((self.y[0] + self.Vtm_s))
        self.alpha_h = self.alpha_h_OLM((self.y[0] + self.Vtm_s))
        self.alpha_n = self.alpha_n_OLM((self.y[0] + self.Vtm_s))
        self.beta_h = self.beta_h_OLM((self.y[0] + self.Vtm_s))
        self.beta_n = self.beta_n_OLM((self.y[0] + self.Vtm_s))
        self.H_inf = self.H_inf_OLM((self.y[0] + self.Vtm_s))
        self.tau_H = self.tau_H_OLM((self.y[0] + self.Vtm_s))
        self.KCa_fonc = self.KCa_fonc_OLM(self.y[3])

        self.I_Na = self.I_Na_OLM(self.g_Na, self.m_inf_Na, self.y[1], (self.y[0] + self.Vtm_s), self.E_Na)
        self.I_K = self.I_K_OLM(self.g_K, self.y[2], (self.y[0] + self.Vtm_s), self.E_K)
        self.I_Ca = self.I_Ca_OLM(self.g_Ca, self.m_inf_Ca, (self.y[0] + self.Vtm_s), self.E_Ca)
        self.I_KCa = self.I_KCa_OLM(self.g_KCa, self.KCa_fonc, (self.y[0] + self.Vtm_s), self.E_K)
        self.I_h = self.I_h_OLM(self.g_h, self.y[4], (self.y[0] + self.Vtm_s), self.E_K)
        self.I_leak = self.I_leak_OLM(self.g_leak, self.PERCENT, (self.y[0] + self.Vtm_s), self.E_leak)

        # self.I_soma_stim = [self getI_soma_stim];

        # ajout IKNa definir y

        self.I_KNa = self.I_KNa_OLM(self.g_KNa, self.y[5], (self.y[0] + self.Vtm_s), self.E_K)
    ...
    #     self.y[0] += np.sqrt(self.dt)*self.bruitGaussien(0,1)*self.y[0]*self.Noise
