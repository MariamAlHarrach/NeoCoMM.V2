...
# I_h: hyperpolarisation activated current
    ...
    def __init__(self,Layer,plasticity=0):
        self.Type = 1
        self.Layer = Layer
        # parameter from GUI
        self.E_leak = -70
        ...
        self.I_synSoma = 0.0
        self.I_synDend = 0.0
        self.I_NMDA_CA1 = 0.0 #for calcium concentration (plasticity)
        self.I_synAis = 0.0
        self.I_AMPA_CA1 = 0.0
        self.I_GABA_CA1 = 0.0
        self.s_GABA_CA1 = 0.0
        ...
        self.theta_d = 0.3  #0.3                # 1 Hybrid # 0.3 shouval #0.2 pour dep, the value is in M, later we multply bith theta_d*1e-3 to become mM as everywhere else it's in mM in the code
        self.theta_p = 0.55 #0.55                # 1.3 Hybrid  # 0.55 shouval #0.35 pour pot
        self.beta_1 = 80 #80
        self.beta_2 = 80 #80
        self.A = 1  #550 # 5  # 1               # 550 Hybrid
        self.Bb = 0.25   #250  # 1 # 0.25         # 250 Hybrid
        self.omega0 = 0.25 #0.25  # 1 # 0.25     =0 in case of Hybrid
        self.Lamda = 1
        self.W_max = 1
        self.TauxCa = 100
    ...
    def projection_E(self,E_d, E_s, E_a):
        ...
        if self.Lambda_d[0] == 0 and self.Lambda_d[1] == 0 and self.Lambda_d[2] == 0:
            self.proj_mod_d = 0
        else:
            projection = self.Lambda_d * np.dot(self.E_d, self.Lambda_d) / np.dot(self.Lambda_d, self.Lambda_d)
            norm_projection = np.linalg.norm(projection)*np.linalg.norm(self.Lambda_d)
        ...
        if self.Lambda_s[0] == 0 and self.Lambda_s[1] == 0 and self.Lambda_s[2] == 0:
            self.proj_mod_s = 0
        else:
            projection = self.Lambda_s * np.dot(self.E_s, self.Lambda_s) / np.dot(self.Lambda_s, self.Lambda_s)
            norm_projection = np.linalg.norm(projection)*np.linalg.norm(self.Lambda_s)
            projection_moi_vect = projection + self.Lambda_s
            norm_vect_projectOnto = np.linalg.norm(projection_moi_vect)
            signe = np.sign(norm_vect_projectOnto - norm_projection)
            self.proj_mod_s = norm_projection * signe

        if self.Lambda_a[0] == 0 and self.Lambda_a[1] == 0 and self.Lambda_a[2] == 0:
            self.proj_mod_a = 0
        else:
            projection = self.Lambda_a * np.dot(self.E_a, self.Lambda_a) / np.dot(self.Lambda_a, self.Lambda_a)
            norm_projection = np.linalg.norm(projection)*np.linalg.norm(self.Lambda_a)
    ...
    def update_EField_Stim2(self,f):

        self.Vtm_d =  np.dot(self.Lambda_d, float64(f)) * self.OnOff
        self.Vtm_s =  np.dot(self.Lambda_s, float64(f)) * self.OnOff
        self.Vtm_a =  np.dot(self.Lambda_a, float64(f)) * self.OnOff
    ...
    def updateParameters(self):
        ...
        self.fonc_ghk_s = self.fonc_ghk_CA1(self.y[0]+ self.Vext,self.y[12])
        self.fonc_ghk_d = self.fonc_ghk_CA1(self.y[1],self.y[14])
        self.h_inf_s = self.h_inf_CA1(self.y[0]+ self.Vext)
        self.m_inf_s = self.m_inf_CA1(self.y[0]+ self.Vext)
        self.m_KDR_inf_s = self.m_KDR_inf_CA1(self.y[0]+ self.Vext)
        ...
        self.m_AHP_BK_inf_s = self.m_AHP_BK_inf_CA1(self.y[0]+ self.Vext,self.y[12])
        self.tau_m_AHP_BK_s = self.tau_m_AHP_BK_CA1(self.y[0]+ self.Vext,self.y[12])
        ...
        self.m_m_inf_s = self.m_m_inf_CA1(self.y[0]+ self.Vext)
        self.tau_m_m_s = self.tau_m_m_CA1(self.y[0]+ self.Vext)
        self.m_m_inf_d = self.m_m_inf_CA1(self.y[1])
        self.tau_m_m_d = self.tau_m_m_CA1(self.y[1])

        self.I_Na_s = self.I_Na_CA1(self.g_Na_s, self.y[4], self.y[2], self.y[0]+ self.Vext, self.E_Na)
        self.I_KDR_s = self.I_KDR_CA1(self.g_KDR_s, self.y[3], self.y[0]+ self.Vext, self.E_K)
        ...
        self.I_AHP_s = self.I_AHP_CA1(self.g_AHP_s, self.y[18], self.y[0]+ self.Vext, self.E_K)
        self.I_AHP_d = self.I_AHP_CA1(self.g_AHP_d, self.y[19], self.y[1], self.E_K)
        self.I_AHP_BK_s = self.I_AHP_BK_CA1(self.g_AHP_BK_s, self.y[20], self.y[0]+ self.Vext, self.E_K)
        self.I_AHP_BK_d = self.I_AHP_BK_CA1(self.g_AHP_BK_d, self.y[21], self.y[1], self.E_K)
        self.I_m_s = self.I_m_CA1(self.g_m_s, self.y[22], self.y[0]+ self.Vext, self.E_K)
        self.I_m_d = self.I_m_CA1(self.g_m_d, self.y[23], self.y[1], self.E_K)
        self.I_leak_s = self.I_leak_CA1(self.g_leak_s, self.PERCENT, self.y[0]+ self.Vext, self.E_leak)
        ...
        self.I_KNa = self.I_KNa_CA1(self.g_KNa, self.y[24], self.y[0]+ self.Vext, self.E_K)
    ...
    def derivT(self):
        ...
        self.dVs_dt = 1. / self.Cm * (-self.I_KNa - self.I_Na_s - self.I_KDR_s - self.I_CaL_s - self.I_AHP_s - self.I_AHP_BK_s - self.I_m_s - self.I_leak_s - self.I_synSoma - (self.g_c / self.p_SD * ((self.Vs_+self.Vtm_s + self.Vext)- (self.Vd_+self.Vtm_d))) - (self.g_c / self.p_SA * ((self.Vs_+self.Vtm_s + self.Vext)- (self.Va_+self.Vtm_a))) + self.I_soma_stim)
        self.dVd_dt = 1. / self.Cm_d * (-self.I_Na_d - self.I_KDR_d - self.I_KA_d - self.I_CaT_d - self.I_CaR_d - self.I_h_d - self.I_AHP_d - self.I_AHP_BK_d - self.I_m_d - self.I_leak_d - self.I_synDend - (self.g_c / (1 - self.p_SD) * (-(self.Vs_+self.Vtm_s + self.Vext) + (self.Vd_+self.Vtm_d))) + self.I_dend_stim)
        self.dVa_dt = 1. / self.Cm * (-self.I_Na_a - self.I_KDR_a -self.I_leak_a  - self.I_synAis - (self.g_c /(1-self.p_SA) * ((self.Va_+self.Vtm_a)- (self.Vs_+self.Vtm_s +self.Vext ))) + self.I_axon_stim)
        ...
        self.dCa_d_dt = self.drive_channel_d + ((0.0001 - self.Ca_d_) / self.TauxCa) * 1. + (self.I_NMDA_CA1 * 1e-6) / (2 * self.F)      #Shouval-Calcium dynamics
        ###################################################################################################################
        ## Synaptic Weight variation
        if self.plasticity==0:
            self.dW_dt = 0   #1
        elif self.plasticity==1:
    ...
    # def I_NMDA2(self,Vpre,PreSynaptic_Soma_Dend_AMPA,t):
    #     Vd = np.array([self.VSoma() if s_d == 1 else self.VDend() for s_d in PreSynaptic_Soma_Dend_AMPA])
    #     return np.array([self.computeI_NMDA(Vpre[k], t, Vd[k]) for k in range(len(Vpre))]) #tableau en microA/cm^2
    ...
    #     return self.Ra * B * self.g_NMDA * (Vs_d - self.E_NMDA)*self.y[29]
    ...
    def computeI_NMDA(self,Vpre,t,Vs_d):
        ...
        return self.Ra * self.B * self.g_NMDA * (Vs_d - self.E_NMDA)* self.y[29]    # Computes I_NMDA new model which is multiplied by the weight function, y[29] = W
...
# Synaptic Weight Functions
    ...
    def omega_p(self, Ca):
        return self.A * self.sig_func(Ca - self.theta_p, self.beta_2)

    def omega_d(self, Ca):
        return self.Bb * self.sig_func(Ca - self.theta_d, self.beta_1)

    def omega_func(self, Ca):
        return self.omega0 + self.omega_p(Ca) - self.omega_d(Ca)
    ...
    def eta_func(self, Ca):
                #  , eta_max=1.0, eta_min=0.001, Ca_half=0.5, k=10):
        # return (1/(1 + np.exp(self.eta_A*(0.5-Ca))) )# Ca : mM
        return (100/((100*10**(-4)) + Ca**(3)) +1000)**(-1) # Function of shouval
        #return (100/((100*10**(-4)) + Ca**(3)) +1000)**(-1) # Gabriel's comment. Multiplication instead pf division
#################################################################################################################################
